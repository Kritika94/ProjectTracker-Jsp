 ...
 * @author Kritika
 */
@WebServlet(urlPatterns = {"/SaveAssignee"})
public class SaveAssignee extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String Cumin_number = request.getParameter("value");
        String Assignee = request.getParameter("id");
        try {
            if (JDBCConnectivity.cn1 == null) {
                JDBCConnectivity.Connect();
            }
            JDBCConnectivity.ps = JDBCConnectivity.cn1.prepareStatement("update cumin_ticket set Assignee=? where Cumin_number=?");
            JDBCConnectivity.ps.setString(1, Assignee);
            JDBCConnectivity.ps.setString(2, Cumin_number);
